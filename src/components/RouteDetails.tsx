...
import { ArrowLeft, MapPin, Droplets, User, Phone, Clock, Navigation, CheckCircle, ShoppingCart, UserX, AlertCircle, DollarSign, Calendar } from 'lucide-react';
        ...
        {deliveries.map((delivery, index) => {
          ...
          <Card 
            ...
            className={`hover:shadow-lg transition-all duration-200 border-2 ${
              checkInStatus ? checkInStatus.color : (delivery.status === 'completed' ? 'bg-green-50/50' : 'hover:scale-[1.01]')
            }`}
                ...
                <div className="space-y-1">
                  <div className="flex items-center space-x-2">
                    <div 
                      ...
                      style={{ background: checkInStatus 
                        ? (checkInStatus.checkInStatus === 'delivered' ? 'linear-gradient(135deg, #10b981 0%, #059669 100%)' : 
                           checkInStatus.checkInStatus === 'no-sale' ? 'linear-gradient(135deg, #6b7280 0%, #4b5563 100%)' :
                           checkInStatus.checkInStatus === 'absent-return' ? 'linear-gradient(135deg, #eab308 0%, #ca8a04 100%)' :
