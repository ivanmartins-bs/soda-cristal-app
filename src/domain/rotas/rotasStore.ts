...
interface RotasState {
  ...
  searchTermByRoute: Record<string, string>; // chave = routeId
  ...
  persist(
    (set, get) => ({
      ...
      loadRotas: async (vendedorId: number, forceRefresh = false) => {
        ...
        const cacheAge = state.lastFetchRotas
          ? now - state.lastFetchRotas
          : Infinity;

        const validCache = cacheAge < CACHE_MINUTES * 60 * 1000;
        const staleCache =
          cacheAge >= CACHE_MINUTES * 60 * 1000 &&
          cacheAge < STALE_HOURS * 60 * 60 * 1000;
      ...
      loadTodaysRoutes: async (vendedorId: number, forceRefresh = false) => {
        ...
        const isNewDay = state.lastFetchDate !== today;
        const cacheAge = state.lastFetchTodaysRoutes
          ? now - state.lastFetchTodaysRoutes
          : Infinity;
        ...
        if (!useNetworkStore.getState().isOnline) {
          ...
          set({
            ...
            loadingProgress: null,
            error:
              "Sem conexão. Sincronize com a internet para baixar as entregas do dia.",
            offlineModeHint: null,
          });
          return;
        }
        ...
        try {
          ...
          const todaysRoutes = await rotasService.getTodaysRoutes(
            vendedorId,
            cachedRotas,
          );
          ...
          if (shouldShowLoading) {
            set({
              loadingStep: "clientes",
              loadingProgress: { current: 0, total: 1 },
            });
          }

          const rotaIds = todaysRoutes.map((r) => r.id);
          const { flat: allClientes, porRota } =
            await rotasService.getClientesParaRotas(rotaIds);
